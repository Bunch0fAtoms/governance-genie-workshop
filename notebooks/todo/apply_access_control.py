...
# MAGIC
# MAGIC Runs on demand (or every 15 minutes as a scheduled job).
# MAGIC Reads the access_control table and reconciles to Unity Catalog (UC) grants:
# MAGIC - Active rows (not expired): GRANT
# MAGIC - Expired rows: REVOKE and mark _active = false
# MAGIC - Logs all actions to the audit table
...
# Collect active, non-expired rows
...
for row in active_rows:
    ...
    try:
        # TODO: Build and log the GRANT statement for each active row
        # Step 1: Build the GRANT SQL string:
        #   f"GRANT {privilege} ON {securable_type} {securable_name} TO {principal_type} '{principal_id}'"
        # Step 2: Print it (the workshop skips actual execution to avoid needing admin grants,
        #   but in production you would call spark.sql(grant_stmt))
        # Step 3: Log the grant action to the audit table using spark.sql(f"INSERT INTO ...")
        #   Columns: action_time (current_timestamp()), action ('grant'), principal_id, principal_type,
        #            securable_type, securable_name, privilege,
        #            reason ('Idempotent apply cycle'), executed_by ('apply_access_control_job')

        # === YOUR CODE HERE ===
        print(f"[TODO] GRANT: {principal_id} -> {securable_name}:{privilege}")
        # === END TODO ===
...
for row in expired_rows:
    ...
    try:
        # TODO: Build and log the REVOKE statement for each expired row
        # Step 1: Build the REVOKE SQL string:
        #   f"REVOKE {privilege} ON {securable_type} {securable_name} FROM {principal_type} '{principal_id}'"
        # Step 2: Print it (same pattern as grants above)
        # Step 3: Log the revoke action to the audit table (action = 'revoke', reason = 'Access expired')
        # Step 4: Mark the row as inactive in access_control:
        #   spark.sql(f"""UPDATE {catalog}.workshop_governance.access_control
        #     SET _active = false
        ...
        # === YOUR CODE HERE ===
        print(f"[TODO] REVOKE: {principal_id} <- {securable_name}:{privilege} (expired)")
        # === END TODO ===
